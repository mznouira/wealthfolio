/**
 * Pure TypeScript SHA-256, synchronous, zero dependencies.
 *
 * Used by `hash.ts` so that `importHash` stays synchronous and does not depend on
 * a specific runtime's crypto API. FIPS 180-4.
 */
...
export function sha256Hex(input: string): string {
  ...
  // Padding: 0x80, then zeros, then 64-bit length (big-endian).
